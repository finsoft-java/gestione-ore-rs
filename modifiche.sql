...
ALTER TABLE `partecipanti_globali` ADD `MATRICOLA` VARCHAR(20) NOT NULL;

ALTER TABLE `progetti_commesse` ADD `DATA_INIZIO` DATE NOT NULL AFTER `COD_COMMESSA`, ADD `DATA_FINE` DATE NOT NULL AFTER `DATA_INIZIO`;
ALTER TABLE `progetti_commesse`
  DROP PRIMARY KEY,
   ADD PRIMARY KEY(
     `ID_PROGETTO`,
...
ALTER TABLE progetti ADD `GIUSTIFICATIVO` LONGBLOB NULL ,
  ADD `GIUSTIFICATIVO_FILENAME` VARCHAR(255) NULL;

ALTER TABLE `commesse`
  DROP `GIUSTIFICATIVO`,
  DROP `GIUSTIFICATIVO_FILENAME`;
ALTER TABLE `commesse` ADD `DATA_INIZIO` DATE NOT NULL AFTER `COD_COMMESSA`, ADD `DATA_FINE` DATE NOT NULL AFTER `DATA_INIZIO`;
ALTER TABLE `commesse`
  DROP PRIMARY KEY,
   ADD PRIMARY KEY(
     `COD_COMMESSA`,
...
ALTER TABLE progetti_commesse DROP FOREIGN KEY progetti_commesse_ibfk_2`;`
ALTER TABLE `progetti_commesse` ADD CONSTRAINT `progetti_commesse_ibfk_2` FOREIGN KEY (`COD_COMMESSA`, `DATA_INIZIO`, `DATA_FINE`) REFERENCES `commesse`(`COD_COMMESSA`, `DATA_INIZIO`, `DATA_FINE`) ON DELETE RESTRICT ON UPDATE RESTRICT; 
